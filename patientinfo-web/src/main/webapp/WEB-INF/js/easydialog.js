...
var dialogBox = function(){
	...
	return (function(){
		...
		var setFollow = function(node){
			dialogBox.style.top = (todo.getOffset(node).top + options.followY) + 'px';
			dialogBox.style.left = (todo.getOffset(node).left + options.followX) + 'px';
			options.fixed = false;
...
var base = {
	open : function(){
		...
		// 将自定义的参数与默认参数进行合并
		options = todo.getOptions(arguments[0]);
		...
		if(options.isOverlay){
			...
			if(isIE6){
				todo.appendIframe(layer);
			}
		}
		
		if(isIE6) todo.setBodyBg();	
	...
	close : function(){
		...
		if(document.getElementById('overlay')){
			todo.removeNode(doc.getElementById('overlay'));
		}
		elem.style.display = 'none';
		body.appendChild(elem);
		todo.removeNode(doc.getElementById('dialog_box'));
